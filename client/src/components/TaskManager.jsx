...
export default function TaskManager() {
  ...
  async function handleToggleStatus(task) {
    ...
    setViewingTask((v) =>
      v && v.id === task.id ? { ...v, status: nextStatus } : v,
    );
    await loadTasks();
  }
    ...
    <div className="min-h-screen w-full">
      ...
      <PageShell>
        <PageHeader
          ...
          description={
            user
              ? `Welcome back, ${user.name.split(" ")[0]}. Here's what's on your list.`
              : undefined
          }
          actions={
        ...
        <div className="mb-5 grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            ...
            tone="violet"
            value={`${stats.rate}%`}
            label="Completion rate (this page)"
          />
        </div>
            ...
            <button
              ...
              className={
                ...
                (statusFilter === f.value
                  ...
                  : "border-border text-muted hover:text-ink-soft")
              }
            >
              {f.label}
            </button>
        ...
        <Card>
          <CardHeader
            title="All tasks"
            action={<Badge tone="muted">{meta.total}</Badge>}
          />
              ...
              <Table>
                ...
                <TBody>
                  {tasks.map((task) => (
                    ...
                    >
                      <TD>
                        <span className="block max-w-[360px] truncate font-medium text-ink">
                          {task.title}
                        </span>
                        {task.description && (
                          <span className="mt-0.5 block max-w-[360px] truncate text-[12px] text-muted">
                            {task.description}
                          </span>
                        )}
                      </TD>
        ...
        <Drawer
          ...
          description={
            deletingTask
              ? `Delete "${deletingTask.title}"? This can't be undone.`
              : undefined
          }
          footer={
